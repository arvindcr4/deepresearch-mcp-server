...
const LOG_FORMAT: LogFormat = (process.env.LOG_FORMAT as LogFormat) || 'pretty'
...
if (
  ...
  console.error(
    `FATAL: Calculated logs directory "${resolvedLogsDir}" is outside the project root "${projectRoot}". Logging to files will be disabled.`
  )
  ...
  private constructor() {
    ...
    if (isLogsDirSafe) {
      try {
        ...
        console.error(
          `Error ensuring logs directory exists at ${resolvedLogsDir}: ${error.message}. File logging might be affected.`
        )
    ...
    if (isLogsDirSafe) {
      ...
      console.warn(
        'File logging is disabled due to unsafe or inaccessible logs directory.'
      )
    }
