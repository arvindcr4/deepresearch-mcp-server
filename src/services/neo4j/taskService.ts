...
export class TaskService {
  ...
  static async createTask(
    ...
    try {
      ...
      const validatedTask = validateData(
        {
          ...task,
          id: taskId,
          urls: task.urls || [],
      ...
      const params: Record<string, any> = {
        ...
        urls: JSON.stringify(validatedTask.urls), // Serialize urls
        tags: validatedTask.tags,
        completionRequirements: validatedTask.completionRequirements,
        outputFormat: validatedTask.outputFormat,
  ...
  static async linkTaskToKnowledge(
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error('Error linking task to knowledge item', {
        error: errorMessage,
        taskId,
        knowledgeId,
        relationshipType,
  ...
  static async areAllDependenciesCompleted(taskId: string): Promise<boolean> {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error('Error checking task dependencies completion status', {
        error: errorMessage,
        taskId,
      })
      throw error
  ...
  static async updateTask(
    ...
    try {
      ...
      const setClauses = ['t.updatedAt = $updatedAt']
  ...
  static async removeTaskDependency(dependencyId: string): Promise<boolean> {
    ...
    try {
      ...
      if (result) {
        ...
        logger.warn('Task dependency not found or not removed', {
          dependencyId,
        })
      }
  ...
  static async getTaskDependencies(taskId: string): Promise<{
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error('Error getting task dependencies', {
        error: errorMessage,
        taskId,
      })
      throw error
