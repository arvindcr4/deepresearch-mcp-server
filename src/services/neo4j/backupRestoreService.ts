...
const manageBackupRotation = async (): Promise<void> => {
  ...
  try {
    ...
    logger.error(`Error during backup rotation management: ${errorMsg}`, {
      error,
    })
...
// Using FullExport type from neo4jValidation.ts
...
export const importDatabase = async (backupDirInput: string): Promise<void> => {
  ...
  try {
    ...
    if (fullExportPath && existsSync(fullExportPath)) {
      ...
      // Import from full export file with validation
      const fullExportContent = readFileSync(fullExportPath, 'utf-8') // Safe to read validated path
      const parsedExport = safeJsonParse(
        fullExportContent,
        fullExportSchema,
        'full export file'
      )
      // Ensure relationships have properties field
      const fullExport: FullExport = {
        ...parsedExport,
        relationships: parsedExport.relationships.map((rel) => ({
          ...rel,
      ...
      for (const nodeFile of nodeFiles) {
        ...
        let nodesToImport: Record<string, any>[]
        try {
          const parsed = JSON.parse(fileContent)
          // Basic validation that it's an array
          if (!Array.isArray(parsed)) {
            throw new Error('File content must be an array of nodes')
          }
          nodesToImport = parsed
        } catch (error) {
          logger.error(
            `Failed to parse ${nodeFile}:`,
      ...
      if (relFilePath && existsSync(relFilePath)) {
        ...
        try {
          const parsed = JSON.parse(relFileContent)
          const parsedRelationships = validateData(
            parsed,
            relationshipDataSchema.array(),
            'relationships file'
          )
          relationships = parsedRelationships.map((rel) => ({
            ...rel,
            properties: rel.properties || {},
          }))
        } catch (error) {
          logger.error(
            'Failed to parse relationships file:',
            error instanceof Error ? error : new Error(String(error))
          )
          relationships = []
        }
        ...
        try {
          await session.executeWrite(async (tx) => {
            ...
            for (const rel of batch) {
              ...
              // Validate relationship type before using it
              if (!ALLOWED_RELATIONSHIP_TYPES.has(rel.type)) {
                logger.warn(
                  `Skipping relationship with unknown type: ${rel.type}. Add to ALLOWED_RELATIONSHIP_TYPES if valid.`
                )
                failedCount++
                continue
              }
