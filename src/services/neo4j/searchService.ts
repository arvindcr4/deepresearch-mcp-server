...
import { escapeRegexValue } from './cypherSecurity.js'
import { executeReadWithRetry } from './transactionWrapper.js'
...
export class SearchService {
  ...
  static async search(
    ...
    PaginatedResult<SearchResultItem> & { partialFailures?: string[] }
  > {
    try {
      const {
        property = '',
      ...
      const escapedValue = escapeRegexValue(value) // Use secure regex escaping
      ...
      // Track errors and partial results
      ...
      settledResults.forEach((result, index) => {
        ...
        if (
          ...
          errors.push({ label, error: errorMessage })
          logger.error(`Search promise rejected for label "${label}":`, {
            reason: errorMessage,
          })
      ...
      // If all searches failed, throw an aggregated error
      if (successfulSearches === 0 && errors.length > 0) {
        const aggregatedError = `All search operations failed: ${errors
      ...
      if (errors.length > 0) {
        logger.warn('Some search operations failed', {
          successfulSearches,
      ...
      const paginatedResult = Neo4jUtils.paginateResults(allResults, {
        page,
        limit,
      })
      ...
      if (errors.length > 0) {
        return {
          ...paginatedResult,
          partialFailures: errors.map((err) => `${err.label}: ${err.error}`),
        } as PaginatedResult<SearchResultItem> & { partialFailures: string[] }
      }
      ...
      logger.error('Error performing unified search', {
        error: errorMessage,
        options,
      })
      throw error
  ...
  private static async searchSingleLabel(
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error(`Error searching label ${labelInput}`, {
        error: errorMessage,
        searchValue: cypherSearchValue,
        normalizedProperty,
      })
    ...
    try {
      ...
      if (targetLabels.includes('project')) {
        ...
        `
          ...
          logger.error('Error during project full-text search query', {
            error: err,
            searchValue,
          })
      ...
      if (targetLabels.includes('task')) {
        ...
        `
          ...
          logger.error('Error during task full-text search query', {
            error: err,
            searchValue,
          })
        } finally {
      ...
      if (targetLabels.includes('knowledge')) {
        ...
        `
          ...
          logger.error('Error during knowledge full-text search query', {
            error: err,
            searchValue,
          })
        } finally {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error('Error performing full-text search', {
        error: errorMessage,
        searchValue,
        options,
      })
