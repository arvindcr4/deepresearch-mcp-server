...
export class Neo4jUtils {
  ...
  static async initializeSchema(): Promise<void> {
    const timeoutMs = 60000 // 60 second timeout for schema initialization
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(
        () =>
          reject(new Error('Schema initialization timeout after 60 seconds')),
        timeoutMs
      )
    })

    const schemaPromise = this.initializeSchemaInternal()

    try {
      await Promise.race([schemaPromise, timeoutPromise])
    } catch (error) {
      if (error instanceof Error && error.message.includes('timeout')) {
        logger.error('Neo4j schema initialization timed out', { timeoutMs })
      }
      throw error
    }
  }

  private static async initializeSchemaInternal(): Promise<void> {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error('Failed to initialize Neo4j database schema', {
        error: errorMessage,
      })
      throw error
    } finally {
  ...
  static async isDatabaseEmpty(): Promise<boolean> {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error('Error checking if database is empty', {
        error: errorMessage,
      })
