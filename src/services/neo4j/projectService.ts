...
import { validatePropertyValue } from './cypherSecurity.js'
import {
  safeJsonParse,
  safeJsonParseArray,
  urlObjectSchema,
  projectDataSchema,
  validateData,
} from '../../schemas/neo4jValidation.js'
import {
...
export class ProjectService {
  ...
  static async createProject(
    ...
    try {
      ...
      // Validate all property values before storing
      validatePropertyValue(project.name, 'name')
      validatePropertyValue(project.description, 'description')
      validatePropertyValue(project.status, 'status')
      validatePropertyValue(
        project.completionRequirements,
        'completionRequirements'
      )
      validatePropertyValue(project.outputFormat, 'outputFormat')
      validatePropertyValue(project.taskType, 'taskType')

      // Validate project data before storing
      ...
      const params = {
        ...
        urls: JSON.stringify(validatedProject.urls), // Serialize to JSON string
        completionRequirements: validatedProject.completionRequirements,
        outputFormat: validatedProject.outputFormat,
        taskType: validatedProject.taskType,
        createdAt: now,
        updatedAt: now,
      }
  ...
  static async getProjectById(id: string): Promise<Neo4jProject | null> {
    ...
    try {
      ...
      const result = await executeReadWithRetry(session, async (tx) => {
  ...
  static async areAllDependenciesCompleted(
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error('Error checking project dependencies completion status', {
        error: errorMessage,
        projectId,
      })
      throw error
  ...
  static async updateProject(
    ...
    try {
      ...
      const setClauses = ['p.updatedAt = $updatedAt']
  ...
  static async getProjects(
    ...
    try {
      ...
      // Execute count query
      const totalResult = await executeReadWithRetry(session, async (tx) => {
      ...
      // Execute data query
      const dataResult = await executeReadWithRetry(session, async (tx) => {
      ...
      const projects: Neo4jProject[] = dataResult.data.map((record) => {
  ...
  static async addProjectDependency(
    ...
    try {
      ...
      const cycleCheckResult = await executeReadWithRetry(
        session,
        async (tx) => {
  ...
  static async removeProjectDependency(dependencyId: string): Promise<boolean> {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error('Error removing project dependency', {
        error: errorMessage,
        dependencyId,
      })
      throw error
  ...
  static async getProjectDependencies(projectId: string): Promise<{
    ...
    try {
      ...
      const [dependenciesResult, dependentsResult] = await Promise.all([
        executeReadWithRetry(
          session,
          async (tx) => (await tx.run(dependenciesQuery, { projectId })).records
        ),
        executeReadWithRetry(
          session,
      ...
      const dependencies = dependenciesResult.data.map((record) => ({
        id: record.get('id'),
        sourceProjectId: record.get('sourceProjectId'),
        targetProjectId: record.get('targetProjectId'),
        type: record.get('type'),
      ...
      const dependents = dependentsResult.data.map((record) => ({
        id: record.get('id'),
        sourceProjectId: record.get('sourceProjectId'),
        targetProjectId: record.get('targetProjectId'),
        type: record.get('type'),
