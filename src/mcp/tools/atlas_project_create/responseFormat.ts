  ...
  implements ResponseFormatter<BulkProjectResponse>
{
  format(data: BulkProjectResponse): string {
    ...
    const summary =
      `${success ? 'Projects Created Successfully' : 'Project Creation Completed with Errors'}\n\n` +
    ...
    if (errors.length > 0) {
      ...
      errorsSection += errors
        .map((error, index) => {
          const projectName =
            error.project?.name || `Project at index ${error.index}`
          return (
            `${index + 1}. Error in "${projectName}"\n\n` +
