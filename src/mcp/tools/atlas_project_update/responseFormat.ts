  ...
  implements ResponseFormatter<SingleProjectResponse>
{
  format(data: SingleProjectResponse): string {
    ...
    if (projectData.description)
      details += `Description: ${projectData.description}\n`
    if (projectData.status) details += `Status: ${projectData.status}\n`

    // Format URLs array
    if (projectData.urls) {
      const urlsValue =
        Array.isArray(projectData.urls) && projectData.urls.length > 0
          ? JSON.stringify(projectData.urls)
          : 'None'
      details += `URLs: ${urlsValue}\n`
    }

    if (projectData.completionRequirements)
      details += `Completion Requirements: ${projectData.completionRequirements}\n`
    if (projectData.outputFormat)
      details += `Output Format: ${projectData.outputFormat}\n`
  ...
  implements ResponseFormatter<BulkProjectResponse>
{
  format(data: BulkProjectResponse): string {
    ...
    const summary =
      `${success ? 'Projects Updated Successfully' : 'Project Updates Completed with Errors'}\n\n` +
    ...
    if (updated.length > 0) {
      ...
      updatedSection += updated
        .map((project, index) => {
          ...
          return (
            `${index + 1}. ${projectData.name || 'Unnamed Project'}\n\n` +
    ...
    if (errors.length > 0) {
      ...
      errorsSection += errors
        .map((error, index) => {
          return (
            `${index + 1}. Error updating Project ID: "${error.project.id}"\n\n` +
...
export function formatProjectUpdateResponse(data: any, isError = false): any {
  ...
  if (isBulkResponse) {
    return createFormattedResponse(
      data,
      new BulkProjectUpdateFormatter(),
      isError
    )
  } else {
    return createFormattedResponse(
      data,
      new SingleProjectUpdateFormatter(),
      isError
    )
  }
}
