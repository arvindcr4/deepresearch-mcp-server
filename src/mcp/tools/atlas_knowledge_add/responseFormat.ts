  ...
  implements ResponseFormatter<BulkKnowledgeResponse>
{
  format(data: BulkKnowledgeResponse): string {
    ...
    const summary =
      `${success ? 'Knowledge Items Added Successfully' : 'Knowledge Addition Completed with Errors'}\n\n` +
    ...
    if (created.length > 0) {
      ...
      createdSection += created
        .map((item, index) => {
          ...
          return (
            `${index + 1}. Knowledge Item (${itemData.domain || 'Uncategorized'})\n` +
    ...
    if (errors.length > 0) {
      ...
      errorsSection += errors
        .map((error, index) => {
          ...
          return (
            `${index + 1}. Error in Knowledge Item for Project "${itemProject}"\n\n` +
...
export function formatKnowledgeAddResponse(data: any, isError = false): any {
  ...
  const isBulkResponse =
    ...
    Object.prototype.hasOwnProperty.call(data, 'created')
