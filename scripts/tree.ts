...
import { compileSafeRegex } from '../dist/utils/safeRegex.js'
...
function isIgnored(
  ...
  for (const { pattern, negated, regex } of ignorePatterns) {
    // Match against the relative path using safe regex compilation
    try {
      const regexPattern = compileSafeRegex(`^${regex}$|^${regex}/`, {
        logRejections: false, // Don't log rejections for gitignore patterns
        timeout: 50,
      })
      ...
      // If pattern is unsafe, skip it and log a warning
      console.warn(`Skipping unsafe gitignore pattern: ${pattern}`)
    }
  }
